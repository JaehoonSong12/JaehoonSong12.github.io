...
d3.csv('./spotify_songs.csv').then(function(data) {
    ...
    // Bind data and create circles for each track
    svg.selectAll("circle")
        .data(filteredData)  // Binding filtered data
        .enter()
        .append("circle")
        .attr("cx", function(d) { return scaleYear(parseDate(d.track_album_release_date)); }) // Scale x position by release date
        .attr("cy", function(d) { return scalePopularityScore(d.track_popularity); }) // Scale y position by popularity score
